/**
 * @description:Given an integer array nums of length n, you want to create an array ans of length 2n where ans[i] == nums[i] and ans[i + n] == nums[i] for 0 <= i < n (0-indexed).

Specifically, ans is the concatenation of two nums arrays.

Return the array ans.

@link https://leetcode.com/problems/concatenation-of-array/description/
 
@example
Example 1:

Input: nums = [1,2,1]
Output: [1,2,1,1,2,1]
Explanation: The array ans is formed as follows:
- ans = [nums[0],nums[1],nums[2],nums[0],nums[1],nums[2]]
- ans = [1,2,1,1,2,1]
Example 2:

Input: nums = [1,3,2,1]
Output: [1,3,2,1,1,3,2,1]
Explanation: The array ans is formed as follows:
- ans = [nums[0],nums[1],nums[2],nums[3],nums[0],nums[1],nums[2],nums[3]]
- ans = [1,3,2,1,1,3,2,1]
 */

var getConcatenation = function (nums) {
  if (!nums) return nums;
  const len = nums.length;
  let ans = [];
  for (let i = 0; i < len; i++) {
    ans[i] = ans[len + i] = nums[i];
  }

  return ans;
};

// let nums = [1, 22, 1];
// console.time(getConcatenation(nums));
// Start the timer
console.time("getConcatenation");

// Define a large input array
const largeInput = Array.from({ length: 1000000 }, (_, index) => index);

// Call the function with the large input
const result = getConcatenation(largeInput);

// Stop the timer and display the elapsed time
console.timeEnd("getConcatenation");

// Optionally, you can log the result to verify correctness or inspect the output
console.log(result);
